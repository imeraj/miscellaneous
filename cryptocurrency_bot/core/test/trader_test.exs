defmodule Naive.TraderTest do
  ...
  test "Placing buy order test" do
    ...
    expect(Strategy.Naive.Leader, :notify, fn %Strategy.Naive.Trader.State{} ->
      send(test_pid, :ok)
      :ok
    end)
    ...
    capture_log(fn ->
      {:ok, trader_pid} = Strategy.Naive.Trader.start_link(trader_state)
