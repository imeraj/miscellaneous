defmodule Strategy.Naive.Leader do
  @moduledoc false
  ...
  alias Core.Repo
  alias Strategy.Naive
  alias Strategy.Naive.Trader
  alias Decimal, as: D

  @binance_client Application.compile_env(:core, :binance_client)

  defmodule State do
    defstruct symbol: nil, settings: nil, traders: []
  end

  defmodule TradeData do
    defstruct pid: nil, ref: nil, state: nil
  end

  # APIs
  def notify(trader_state) do
    GenServer.call(
  ...
  def handle_info(
    ...
    case Enum.find_index(traders, &(&1.pid == trader_pid)) do
      nil ->
        ...
        if settings.status == :shutdown and traders == [], do: Naive.stop_trading(state.symbol)
      ...
      index ->
        new_traders =
          if settings.status == :shutdown do
            ...
            if length(traders) == 1, do: Naive.stop_trading(state.symbol)
  ...
  defp start_new_trader(%Trader.State{} = state) do
    ...
    %TradeData{ref: ref, pid: pid, state: state}
  end
  ...
  defp fetch_symbol_settings(symbol) do
    ...
    settings = Repo.get_by!(Naive.Schema.Setting, symbol: symbol)

    Map.merge(Map.from_struct(settings), symbol_filters)
  end
