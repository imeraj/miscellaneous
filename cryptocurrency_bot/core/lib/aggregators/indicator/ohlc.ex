defmodule Aggregators.Indicator.Ohlc do
  require Logger
  ...
  def process([_ | _] = ohlcs, %TradeEvent{} = trade_event) do
    results = Enum.map(ohlcs, &merge_price(&1, trade_event.price, trade_event.trade_time))

    Enum.map(results, &maybe_broadcast(elem(&1, 0)))
  ...
  def process(symbol, %TradeEvent{} = trade_event) do
    Enum.map(
      ...
      &generate_ohlc(symbol, &1, trade_event.price, trade_event.trade_time)
    )
  end

  # private functions
  defp merge_price(%__MODULE__{} = ohlc, price, trade_time) do
  ...
  defp generate_ohlc(symbol, duration, price, trade_time) do
