defmodule Core.ServiceSupervisor do
  ...
  def autostart_workers(repo, schema, module, worker_module),
    do:
      Enum.map(
        fetch_symbols_to_start(repo, schema),
        &start_worker(&1, repo, schema, module, worker_module)
      )
  ...
  def get_pid(worker_module, symbol) do
    Process.whereis(:"#{worker_module}-#{String.upcase(symbol)}")
  end

  def update_status(symbol, status, repo, schema)
      when is_binary(symbol) and is_atom(status) do
    repo.get_by(schema, symbol: String.upcase(symbol))
